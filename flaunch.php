...
if(isset($_REQUEST['launch']))
{
    ...
    $endpoint = 'https://erysnotes.niallbarr.me.uk/index.php';
    ...
    $ltiform = "<form action='$endpoint' method='post'>";
    foreach($params as $k=>$v)
    {
        if($k != "ext_submit")
            $ltiform .= "<input type='hidden' name='$k' value='$v'/>\n";
    }
    $ltiform .=  "<input type='submit' name='ext_submit' id='ext_submit' value='{$params['ext_submit']}'/>\n</form>";
    $ltiform .=  "<script lang='JavaScript'>document.getElementById('ext_submit').click();</script>";

    $ltiform = htmlspecialchars($ltiform, ENT_QUOTES);

    echo '<!DOCTYPE html><html xml:lang="en"><head><title>Frame launch test</title></head><body style="background-color: #bcd;">';
    echo '<h1>Frame launch test</h1>';
    echo '<iframe srcdoc="'.$ltiform.'" width="99%" height="90vh" style="border:1px solid lightgrey; height: 90vh">Hello</iframe>';
    echo '</body>';
}
else
{
