...
function wiki2html($wikiText, $preview=false, $mediaDir=false, $mediaURL=false, &$files=array())
{
    global $savedBlocks;
    $savedBlocks = array();
	$out = $wikiText;
...
/****** split - conversion of the full markup text into lines to be processed */
    ...
    for($n=0; $n<sizeof($lines); $n++)
    {
    	...
    	//$lines[$n] = preg_replace_callback('%\\[\\[@?[a-zA-Z0-9-_#+=~ ./?&:]+(\\|[a-zA-Z0-9-_#+=.~;?\\\\\\/@, ]+)?\\]\\]%',"link2html2('$0', \$preview)", $lines[$n]);
    	while(preg_match('%\\[\\[@?[a-zA-Z0-9-_#+=~ ./?&:]+(\\|[a-zA-Z0-9-_#+=.~;?\\\\\\/@, ]+)?\\]\\]%', $lines[$n], $matches))
        {
            $rep = link2html2($matches[0], $preview);
            $lines[$n] = str_replace($matches[0], $rep, $lines[$n]);
        }
 ...
 //#   	$lines[$n] = preg_replace_callback('/Attach:([A-Za-z0-9-_]+\\.[A-Za-z0-9]+)("[\\w ]+")?/',"mediaLink('$0', \$preview)", $lines[$n]);
    	while(preg_match('/Attach:([A-Za-z0-9-_\\/]+\\.[A-Za-z0-9]+)("[\\w ]+")?/', $lines[$n], $matches))
        {
            if(!in_array($matches[1], $files))
                $files[] = $matches[1];
            $rep = mediaLink($matches[0], $preview, $mediaDir, $mediaURL);
            $lines[$n] = str_replace($matches[0], $rep, $lines[$n]);
           // lines[$n] = preg_replace('/Attach:([A-Za-z0-9-_\\/]+\\.[A-Za-z0-9]+)("[\\w ]+")?/', $rep, $lines[$n], 1);
        }
	}
    ...
    for($n=0; $n<sizeof($lines); $n++)
    {
    	//$lines[$n] = preg_replace_callback('/%(lfloat|rfloat|lframe|rframe)%(.*)(%%)?/',"putSpan('$1', '$2')", $lines[$n]);
    	while(preg_match('/%(lfloat|rfloat|lframe|rframe)%(.*)(%%)?/', $lines[$n], $matches))
        {
            $rep = putspan($matches[1], $matches[2]);
            $lines[$n] = str_replace($matches[0], $rep, $lines[$n]);
        }
	}
    ...
    for($n=0; $n<sizeof($lines); $n++)
    {
        ...
        if(($blocktype=="ul")||($blocktype=="ol"))
        {
            ...
            if(($prevdepth!=$cdepth)||($blocktype!=$lastBlock))
            {
                ...
                //echo getLT($listtypes, $wdepth).'<br/>';
                while(($wdepth > 0)&&((getLT($listtypes, $wdepth)!=$lt)||($wdepth>$cdepth)))
                {
...
//    $out = preg_replace_callback('/\\[--block_(\\d+)--]/',"\$savedBlocks[$1]['content'];", $out);
    	while(preg_match('/\\[--block_(\\d+)--]/', $lines[$n], $matches))
        {
            $rep = $savedBlocks[$matches[1]]['content'];
            $lines[$n] = str_replace($matches[0], $rep, $lines[$n]);
        }
...
function mediaLink($linkSrc, $preview, $mediaDir, $mediaURL)
{
    ...
    if(strpos($linkSrc, "."))
    	...
    	$type = "";
//    if(checkUploadExists($linkSrc, $mediaDir, $mediaURL))
//    {
      	$location = $mediaURL.'/'.$linkSrc;
        if($alt=="") $alt = $linkSrc;
        switch($type)
        {
        case "jpg":
...
function checkUploadExists($linkSrc, $mediaDir, $mediaURL)
{
    return file_exists($mediaDir.'/'.$linkSrc);
}
