...
abstract class nbform
{
	...
	function formEnd($showSubmit=true, $showCancel=false)    //# include submit/cancel
	{
    	...
    	if($showSubmit|$showCancel)
        {
    		...
    		if($showCancel)
	            $out .= '<input class="submit cancel" name="submit" type="submit" value="Cancel" />';
            $out .= "</div>";
        }
        if($this->inFieldset)
        	$out .= '</fieldset>';
	...
	function submitInput($name, $value1, $value2=null)
	{
        ...
        $out .= '<input class="submit" name="'.$name.'" type="submit" value="'.$value1.'" />';
  		if($value2)
            $out .= '<input class="submit cancel" name="'.$name.'" type="submit" value="'.$value2.'" />';
        $out .= "</div>";;
	    return $out;
	}

	function submitByScriptInput($name, $value1, $useScript=true, $value2=null, $useScript2=true)
	{
	    $out = "<div class=\"formfield\">";
        $type = $useScript ? 'button' : 'submit';
        $out .= '<input class="submit" name="'.$name.'" type="'.$type.'" value="'.$value1.'" />';
  		if($value2)
        {
            $type = $useScript2 ? 'button' : 'submit';
            $out .= '<input class="cancel" name="'.$name.'" type="'.$type.'" value="'.$value2.'" />';
        }
        $out .= "</div>";;
	    return $out;
	}
